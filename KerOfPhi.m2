...
----Making chain complexes.--experimenting code with problems
k=6

N = new ChainComplex; N.ring=R;
M = new ChainComplex; M.ring=R;

N#2 =comodule(iPIedges(k));
M#2 =comodule(iIedges(k));
N#1 =directSum(for v in ivertices list comodule(trim intersect(
          edges := positions(verticesOfEdges_piedges, e -> member(v,e));
	  ids := iedgeIdeals_(piedges_edges);
	  for i in ids list i^k
	  ))
 );
M#1 =directSum(for v in ivertices list comodule(trim intersect(
              edges := positions(verticesOfEdges_iedges, e -> member(v,e));
              ids := iedgeIdeals_edges;
              for i in ids list i^k
	            ))
	    );
N#0 =directSum for e in tiedges list comodule ideal(1_R);
M#0 =directSum for I in iedgeIdeals_tiedges list comodule I^k;
d2=transpose(matrix{{#ivertices : 1_R}});
d1=transpose(promote(submatrix(CSC.dd_1,ivertices,tiedges),R));


N.dd#2 =map(N#1,N#2,d2);
N.dd#1 =map(N#0,N#1,d1);
--(N.dd#1)*(N.dd#2)==0

M.dd#2 =map(M#1,M#2,d2);
M.dd#1 =map(M#0,M#1,d1);
--(M.dd#1)*(M.dd#2)==0

eta=map(N,M,i -> id_(M#i));
NoM=ker eta;

prune HH_2 NoM
hilbertFunction(13,HH_1 NoM)
hilbertSeries(HH_1 NoM)
res HH_1 NoM
ann(HH_1 NoM)
H1NoM5=HH_1 NoM;
